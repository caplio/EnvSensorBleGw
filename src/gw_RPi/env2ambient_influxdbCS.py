...
# influxdb connect setting
from influxdb import InfluxDBClient

    # InfluxDBの接続情報を設定します
    host = 'localhost'  # InfluxDBのホスト
    port = 8086  # InfluxDBのポート
    username = 'your_username'  # InfluxDBのユーザー名
    password = 'your_password'  # InfluxDBのパスワード
    database = 'your_database'  # InfluxDBのデータベース名   
...
def send2ambient(am, dataRow):
   # InfluxDBクライアントを作成します
    ...
    # 測定データを解析してInfluxDBに格納します
    if target == 'esp32':
        (seq, temp, humid, press) = struct.unpack('<Bhhh', dataRow)
        ret = client.write_points([
            {
                'measurement': 'esp32',
                'fields': {
        ...
        am.send({'d1': temp / 100, 'd2': humid / 100, 'd3': press / 10})
    ...
    # InfluxDBとの接続をクローズします
    client.close()

    # Ambientにデータを送信した結果を表示します
